...
def p10(arr):
    arr = np.array(arr)
    a = np.array([2, 4, 1, 6, 3, 9, 0, 8, 7, 5])
...
def p8(arr):
    ...
    pbox = np.array([3, 0, 4, 1, 5, 2, 7, 6])
    reordered_arr = np.zeros_like(arr)
    # 将切片后的 input_array 中的元素按照数组 pbox 中指定的索引重新排列到 reordered_arr 中
...
def initial_permutation(arr):
    ...
    # 例如，reordered_arr 中的第 0 位将被赋值为 arr 中的第 1 位元素
    reordered_arr[a] = arr
    return reordered_arr
